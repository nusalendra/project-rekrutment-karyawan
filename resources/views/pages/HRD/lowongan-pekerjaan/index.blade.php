...
@section('content')
        ...
        <div class="bg-stone-200 bg-auto rounded h-216">
            <div class="px-12 pt-9 text-black">
                <div class="flex justify-between items-center mb-3">
                    <h2 class="flex h-full font-bold text-gray-700 items-center drop-shadow-md text-xl ">Lowongan Pekerjaan
                    </h2>
                    <div class="flex space-x-6 items-center">
                        <a href="/lowongan-pekerjaan/create"
                    ...
                    <table class="w-full text-base text-left text-black dark:text-gray-400">
                        <thead
                            ...
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3">
                                    <h1 class="flex w-full justify-center">Jabatan Kosong</h1>
                                </th>
                                <th scope="col" class="px-6 py-3">
                                    <h1 class="flex w-full justify-center">Kuota Pelamar</h1>
                                </th>
                                <th scope="col" class="px-6 py-3">
                                    <h1 class="flex w-full justify-center">Status Lowongan</h1>
                                </th>
                                <th scope="col" class="px-6 py-3">
                            ...
                            @if (count($data) == 0)
                                <tr
                                    ...
                                    <td class="px-6 py-4" colspan="7">
                                        <h1 class="flex w-full justify-center">Tidak Ada Data Lowongan Pekerjaan</h1>
                                    </td>
                                </tr>
                            @else
                                @foreach ($data as $index => $item)
                                    <tr
                                        ...
                                        <td class="px-6 py-4">
                                            <h1 class="flex w-full justify-center">{{ $item->nama_periode }}</h1>
                                        </td>
                                        <td class="px-6 py-4">
                                        ...
                                        <td class="px-6 py-4">
                                            <h1 class="flex w-full justify-center">{{ $item->kuota }}</h1>
                                        </td>
                                        <td class="px-6 py-4">
                                            <h1 class="flex w-full justify-center">
                                        ...
                                        <td class="px-6 py-4">
                                            <h1 class="flex w-full justify-center">
                                                @php
                                                    $lowonganPekerjaanId = Crypt::encrypt($item->id);
                                                @endphp
                                                {{-- Edit --}}
                                                <a href="/lowongan-pekerjaan/edit/{{ $lowonganPekerjaanId }}"
                                                ...
                                                {{-- Delete --}}
                                                <a href="/lowongan-pekerjaan/delete/{{ $item->id }}"
    ...
    <script>
        ...
        const loadData = async (searchTerm, page) => {
            try {
                const response = await fetch(`/lowongan-pekerjaan?search=${searchTerm}&page=${page}`);
