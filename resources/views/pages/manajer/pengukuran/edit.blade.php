    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <div class="content bg-white bg-auto h-216 mt-2 rounded">
            <div class="px-11 py-9 text-black">
                ...
                <form action="{{ route('pengukuran-update', $pengukuran->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div>
                        <div class="flex mb-1">
                                ...
                                <select id="jabatan_id" name="jabatan_id"
                                    ...
                                    @foreach ($jabatan as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $item->id == $pengukuran->jabatan_id ? 'selected' : '' }}>
                                            {{ $item->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="w-1/2 mx-3">
                                ...
                                <select id="subkriteria_id" name="subkriteria_id"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                    <option disabled>Pilih Subkriteria</option>
                                    @foreach ($subkriteria as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $item->id == $pengukuran->subkriteria_id ? 'selected' : '' }}>
                                            {{ $item->nama }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="flex mb-1">
                            <div class="w-1/2 mx-3 mt-5">
                                ...
                                <input required type="text" id="nama" name="nama"
                                    placeholder="Masukkan Nama Pengukuran" value="{{ $pengukuran->nama }}"
